...
def get_score_autodan_low_memory(tokenizer, conv_template, instruction, target, model, device, test_controls=None,
    ...
    for item in test_controls:
        ...
        if defense_model is not None:
            ...
            newconv.append_message(role=newconv.roles[1], message="")
            newprompt = Prompt(newconv.get_prompt(), perturbable_prompt=jbstr, goal=instruction, target=target)

            logits = defense_model.evaluate_logits(newprompt, model, tokenizer) # pass the parameter model to evaluate loss
